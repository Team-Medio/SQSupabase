...
import { ChannelsPlaylist } from "./EndPoint/ChannelsPlaylists.ts";
...
Deno.serve(async (req) => {
  ...
  const pathname = url.pathname.split("channels")[1];
  if(pathname.startsWith("/playlists")){
    const playlists = new ChannelsPlaylist(supabase);
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/channels' \
