...
import { SqoopsLog } from "./EndPoint/SqoopsLog.ts";
...
Deno.serve(async (req) => {
  ...
  switch(url.pathname){
    case "/sqoops/log": {
      const sqoopsLog = new SqoopsLog(supabase);
      return await sqoopsLog.action(req);
    }
    default: {
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/sqoops' \
