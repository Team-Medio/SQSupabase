...
class EdgeFunctionTester:
    ...
    def test_playlists_ids_most_weekly(self):
        """Chart 플레이리스트 주간 스쿱 차트 테스트"""
        print("📱 Chart 플레이리스트 주간 스쿱 차트 테스트")
        endpoint = "/charts/playlists?filter=most&period=week&date=2025-08-12&limitcount=10"
        return self.make_request(endpoint, "GET")

    def test_playlists_ids_recent(self):
        """Chart 플레이리스트 최근 스쿱 차트 테스트"""
        print("📱 Chart 플레이리스트 최근 스쿱 차트 테스트")
        endpoint = "/charts/playlists?filter=recent&limitcount=10"
        return self.make_request(endpoint, "GET")

    def test_channels_ids_most_weekly(self):
        """Chart 채널 주간 스쿱 차트 테스트"""
        print("📱 Chart 채널 주간 스쿱 차트 테스트")
        endpoint = "/charts/channels?filter=most&period=week&date=2025-08-12&limitcount=10"
        return self.make_request(endpoint, "GET")

    def test_channels_ids_recent(self):
        """Chart 채널 최근 스쿱 차트 테스트"""
        print("📱 Chart 채널 최근 스쿱 차트 테스트")
        endpoint = "/charts/channels?filter=recent&limitcount=10"
        return self.make_request(endpoint, "GET")
    ...
    def interactive_test(self):
        ...
        while True:
            print("사용 가능한 명령어:")
            print("  1. playlist_recent    - PlaylistIDs Recent 테스트")
            print("  2. playlist_most    - PlaylistIDs Most 테스트")
            print("  3. channel_recent    - ChannelIDs Recent 테스트")
            print("  4. channel_most    - ChannelIDs Most 테스트")
            try:
                ...
                if command in ['quit', 'q', 'exit']:
                    print("👋 테스트 종료!")
                    break
                elif command == '1' or command == 'playlist_recent':
                    self.test_playlists_ids_recent()
                elif command == '2' or command == 'playlist_most':
                    self.test_playlists_ids_most_weekly()
                elif command == '3' or command == 'channel_recent':
                    self.test_channels_ids_recent()
                elif command == '4' or command == 'channel_most':
                    self.test_channels_ids_most_weekly()
                else:
                    print("❓ 알 수 없는 명령어입니다.")
